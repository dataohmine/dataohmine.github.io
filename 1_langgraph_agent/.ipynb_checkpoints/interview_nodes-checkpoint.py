...
def load_questions_node(state):
    # Load different question sets based on the role (operator/sponsor)
    role = state["role"]
    stage = state["stage"]
    
    # Define the correct question file paths based on the role and stage
    if role == "operator":
        question_file = f"questions/{stage}.json"  # Corrected file path
    elif role == "sponsor":
        question_file = f"questions/{stage}.json"  # Corrected file path
    else:
...
def handle_special_logic(state, transcription, question):
    ...
    if question["id"] == "name":
        ...
        state["session"]["operator"] = f"{fn}_{ln}"
